...
7. **Server restart (A):** `Ctrl-C`, then start the server again with the same
   `--db`. *Expected:* on restart the host flips to `disconnected`; within a few
   seconds the agent (B, still running, unmodified) reconnects → `connected` again.
...
1. **Server (A), TLS:**
   ```
   /path/to/partout --mode=server --port=8443 --db=/tmp/partout-val/tls.db --tls on
   ```
   *Expected:* `TLS enabled (CA in /tmp/partout-val/tls, SANs …)` then
...
3. **Mint a fresh token (A):** the TLS server has a **separate DB** (`tls.db`), so the
   Variant-1 token is unknown to it:
   ```
   /path/to/partout ctl --server <A_IP>:8443 enroll-token
   ```
4. **Agent (B), mTLS:**
   ...
   ```
   *Expected:* `enrolling with token …` → `enrolled as ag_…` → `TLS material saved …`
   (leaf + key persisted under `<data-dir>/tls/`) → `connected`.
5. **mTLS enforcement (A):** a **plaintext** client should be **rejected** on the TLS
   ...
   `{"status":"ok"}`.
6. **Run a command (A):** `ctl run --selector all -- id` → `succeeded` (runs over the
   mTLS stream).
